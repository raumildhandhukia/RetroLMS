...
const userController = {
  ...
  deleteInstructor:async (req, res) => {
    ...
    try {
      ...
      res.status(200).send({ message: "Instructor and user deleted successfully" });
  } catch (error) {
      console.error("Error deleting instructor:", error);
      res.status(500).send({ message: "Error deleting instructor", error: error.message });
  }
  },
  updateInstructorObject: async (req, res) => {
    ...
    try {
        const updatedInstructor = await User.findOneAndUpdate(
            { username: username },
            { $set: { "profile.firstName": firstName, "profile.lastName": lastName, "profile.email": email, password: password }},
