...
const validateSignInInputs = (credentials: Credentials): ValidationResult => {
  ...
  if (!userIdPattern.test(credentials.username)) {
    return {
      isValid: false,
      message:
        "Username should contain characters, numbers, and @, -, # characters.",
    };
  }
  ...
  if (!passwordPattern.test(credentials.password)) {
    return {
      isValid: false,
      message:
        "Password should be minimum 8 characters long including numbers, and special characters.",
    };
  }
    ...
    >
      <div className="flex flex-col items-center w-full max-w-[400px] rounded-lg shadow border-2 border-black bg-gray-100">
        ...
        <div className="form-container p-6">
          ...
          >
            <button
              className="bg-asu-maroon/90 hover:bg-asu-maroon p-2 rounded-md text-white mt-4"
              type="submit"
            >
              Sign In
            </button>
